...
import { Flex, Text, Skeleton } from '@shrimpswap/uikit'
...
const FarmCard: React.FC<FarmCardProps> = ({ farm, removed, cakePrice, bnbPrice, ethPrice, ethereum, account }) => {
  ...
  const { stakedBalance } = useFarmUser(farm.key)
  ...
  const earnLabel = farm.whale ? 'WHALE' : 'SHRIMP'
  const farmAPY =
    farm.apy &&
  ...
  return (
    <FCard>
      {farm.tokenSymbol === 'SHRIMP' && <StyledCardAccent />}
      <CardHeading
        lpLabel={lpLabel}
      ...
      <Flex justifyContent="space-between">
        ...
        <Text bold style={{ display: 'flex', alignItems: 'center' }}>
          {getBalanceNumber(new BigNumber(farm.depositFeeBP).div(100)).toFixed(1)}%
        </Text>
      </Flex>
